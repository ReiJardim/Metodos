...
# %%
x_target = 0.25
f_025_calculated = func_f(x_target)

print(f"O valor calculado de f({x_target}) é: {f_025_calculated:.7f}")

# %% [markdown]
# ## Parte ii): Calcular P₂(0.25) usando Polinômio de Interpolação de Lagrange de 2º Grau
#
...
# Alternativa 1: x_coords_interp = [0.1, 0.2, 0.3]
# Alternativa 2 (escolhida para este notebook)
x_coords_interp = [0.2, 0.3, 0.4]

# Verificar se os pontos escolhidos estão na tabela
if not all(x in table_e_3x for x in x_coords_interp):
    print(
        f"Erro: Um ou mais dos x_coords_interp ({x_coords_interp}) não estão na tabela fornecida.")
else:
    # Calcular os valores de y = f(x) = x * e^(3x) para os pontos de interpolação,
    # usando os valores de e^(3x) da tabela.
    y_coords_interp = []
    print("Pontos (x_i, f(x_i)) usados para a interpolação:")
...
if 'y_coords_interp' in locals() and len(y_coords_interp) == 3:  # Verifica se y_coords_interp foi definido
    p2_025_interpolated = lagrange_p2(
    ...
    print(
        f"O valor interpolado P₂({x_interpolate_at}) é: {p2_025_interpolated:.7f}")
else:
    print("Não foi possível calcular P₂(0.25) pois os pontos de interpolação não foram definidos corretamente.")
...
# %%
print("Entrega:")
print(f"i) f(0.25): {f_025_calculated:.7f}")
if 'p2_025_interpolated' in locals():
    print(f"ii) P₂(0.25): {p2_025_interpolated:.7f}")
else:
